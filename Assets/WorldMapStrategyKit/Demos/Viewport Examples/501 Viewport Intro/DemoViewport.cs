...
namespace WorldMapStrategyKit {
    public class DemoViewport : MonoBehaviour {
		...
		void Update() {
			if (map.input.GetKeyDown(KeyCode.Space)) showGUI = !showGUI;
		}
...
//		planeLocation = map.calc.toPlaneLocation;
            ...
            handler.OnMarkerMouseDown += (marker, buttonIndex) => Debug.Log("Click on sprite with button " + buttonIndex + "!");
            handler.OnMarkerMouseEnter += (marker) => Debug.Log("Pointer is on sprite!");
            handler.OnMarkerMouseExit += (marker) => Debug.Log("Pointer exits sprite!");
